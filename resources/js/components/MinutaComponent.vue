        ...
        >
          <thead>
            ...
            <tr>
              ...
              >TOTAL EN LIBRAS, LT O UNIDADES</th>
            ...
            <tr>
              <th
                class="text-center"
                v-for="menu in menus"
                :class="[menu.feriado == 1 ? 'exclusion' : '']"
              >{{ menu.menu }}</th>
        ...
        <div class="modal-content">
          <div class="modal-header">
            ...
            <h4 class="modal-title" id="myModalLabel">
              <i class="fa fa-tasks"></i> Imprimir pedido por tipo de producto
            </h4>
          </div>
              ...
              <div class="row">
                <div class="col-lg-6">
                  <div class="form-group">
                    <label
                      for="produc_type_id"
                      class="control-label gcore-label-top"
                    >Tipo de proucto:</label>
                    <el-select
                      v-model="produc_type_id"
                      filterable
...
export default {
  ...
  methods: {
    ...
    getProductType: function() {
      let me = this;
      axios
        .get("../../administracion/tipo_producto/getDataSelect")
    ...
    getUnidadMedida: function() {
      let me = this;
      axios
        .get("../../administracion/unidad_de_medida/getDataSelect")
        .then(function(response) {
    ...
    getProductos: function() {
      let me = this;
      axios
        .get("getProductsMinuta/" + this.uds_id)
        .then(function(response) {
    ...
    saveRemanencia() {
      let me = this;
      this.$validator
        .validateAll(["product_id", "cantidad"])
        .then(result => {
          if (result) {
            axios
              .post("saveRemanencia", {
        ...
        columns: [
          ...
          },
          {
            ...
            render: function(data, type, full, meta) {
              var btn_delete =
                ' <a onclick="eliminarRemanencia(' +
                full.id +
                "," +
                true +
                ")\" class='btn btn-outline btn-danger btn-xs' data-toggle='tooltip' data-placement='top' title='Eliminar'><i class='fa fa-trash'></i></a> ";
              return btn_delete;
            }
          }
        ]
    ...
    datatable: function(id_us) {
      ...
      $("#tbl-cobertura").DataTable({
        ...
        searching: true,
        dom:
          "<'row'<'col-sm-9 text-right'f><'col-sm-3 text-right'B><'floatright'>rtip>",
        buttons: [
          {
            extend: "print",
            text:
              '<i class="fa fa-retweet" aria-hidden="true"></i> Remanencias',
            titleAttr: "Imprimir",
          ...
          },
          {
            extend: "collection",
            text:
              '<i class="fa fa-print" aria-hidden="true"></i> Imprimir <i class="fa fa-angle-double-down" aria-hidden="true"></i>',
            buttons: [
              {
                text:
              ...
              {
                text:
                  '<i class="fa fa-print" aria-hidden="true"></i> Completo con remanencias',
                action: function(e, dt, node, config) {
                  window.open(
                    "../" +
                      me.minuta.id +
              ...
              {
                text:
                  '<i class="fa fa-print" aria-hidden="true"></i> Pedido solo UDS',
                action: function(e, dt, node, config) {
                  window.open(
                    "../" +
                      me.minuta.id +
        ...
        rowCallback: function(row, data, index) {
          ...
          $(row)
            ...
            .css("font-weight", "bold")
            .css("font-size", "15px")
            .css("background-color", "rgb(209, 243, 209)");
          $(row)
            .find("td:eq(16)")
            .css("color", "black")
            .css("font-weight", "bold")
